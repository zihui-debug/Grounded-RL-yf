...
from .sharding_manager.fsdp_vllm_multiturn import FSDPVLLMShardingManager as FSDPVLLMShardingManagerMultiturn
...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if self._is_actor or self._is_critic:
            if optim_config.strategy == "adamw":
                ...
                print(f"Using AnyPrecisionAdamW with bfloat16")
    ...
    def _build_rollout(self) -> None:
        ...
        multiturn = self.config.rollout.multiturn
    ...
    def begin_conversation(self):
        """Synchronise weights & wake the vLLM engine *once* for a full dialogue."""
        if getattr(self, "_ctx_open", False):
            # already open – nothing to do (allows nested calls without harm)
            return
        ...
        # Original code path with sharding manager
    ...
    def generate_step(self, prompts: DataProto):
    ...
    # @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    # def generate_step(self, prompts: DataProto) -> DataProto:
    #     """
    #     Generate **one** turn while the conversation context is open.
    #     No enter/exit overhead here – the engine is already awake.
    #     """
    #     assert getattr(self, "_ctx_open", False), (
    #         "begin_conversation() must be called before generate_step()."
    #     )

    #     # NOTE:  everything below is the same inner‑body you previously had
    #     # inside FSDPWorker.generate_sequences – except for the surrounding
    #     # 'with self.rollout_sharding_manager:'.
    #     if self._use_param_offload:
    #         load_fsdp_model(self.fsdp_module)
    ...
    #     # ----------------------------------------------------------
    #     # preprocess → rollout → postprocess  (NO context‑manager)
    #     # ----------------------------------------------------------
    #     prompts = self.rollout_sharding_manager.preprocess_data(prompts)
    #     output = self.rollout.generate_sequences(prompts=prompts)
    #     output = self.rollout_sharding_manager.postprocess_data(output)

    #     # Optional: re‑offload actor weights & optimiser after each turn
    #     if self._use_param_offload:
    #         offload_fsdp_model(self.fsdp_module)
    ...
    def end_conversation(self):
        """Put vLLM back to sleep and free GPU memory."""
        if not getattr(self, "_ctx_open", False):
            # context already closed (or never opened)
            return

        # Propagate any underlying exceptions out of __exit__
        self.rollout_sharding_manager.exit(None, None, None)
        self._ctx_open = False
        # Any additional clean‑up can go here
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def update_ref_policy_from_actor_weights(self, actor_wg):
        """
        Update reference policy with weights from the provided actor worker group.
        This is used when the actor and ref policies are in separate worker groups.
        
        Args:
            actor_wg: The worker group containing the actor model
        """
        assert self._is_ref
        
        # First load the reference model parameters if they were offloaded
        if self._use_param_offload:
            load_fsdp_model(self.fsdp_module)
        
        # Get state dict from actor (this requires communication)
        actor_state_dict = get_model_state_dict(actor_wg.actor_module)
        
        # # Update ref_policy with actor's weights
        # with FSDP.state_dict_type(self.fsdp_module, StateDictType.FULL_STATE_DICT):
        #     self.fsdp_module.load_state_dict(actor_state_dict)

        set_model_state_dict(
            model=self.fsdp_module,
            model_state_dict=actor_state_dict['model'],
        ...
        # If necessary, offload the parameters again
        if self._use_param_offload:
            offload_fsdp_model(self.fsdp_module)
        
        self.print_rank0("Reference policy updated with current actor weights")
        
        return True

    # @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    # def get_model_state_dict(self):
    #     """Get the state dict of the model for transferring to other workers"""
    #     if self._use_param_offload:
    #         load_fsdp_model(self.fsdp_module)
