...
MODEL="/home/zhaochaoyang/yangfan/project/Grounded-RL-yf/checkpoints/sft/qwen2_5vl-7b/full/mini-o3-coldstart"
NUM_GPUS=8
PORT=8105
# ---------------------------------------------------------------------
...
if [ "$ACTOR_MODEL" == "qwen_vllm" ]; then
  ...
  if lsof -Pi :8105 -sTCP:LISTEN -t >/dev/null 2>&1 || pgrep -f "vllm serve" >/dev/null 2>&1; then
    echo "[INFO] vllm server is already running on port 8105"
  else
  ...
  for i in {1..360}; do
    if curl --max-time 2 -s -o /dev/null http://localhost:8105/v1/models; then
