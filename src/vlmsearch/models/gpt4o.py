...
class GPT4o():
    ...
    def generate_single_thought(
        ...
        if previous_thoughts:
            ...
            for text_str, maybe_image in previous_thoughts:
                if maybe_image is not None:
                    ...
                    text_str = '\n' + text_str
                    messages.append({"role": "user", "content": [{"type": "text", "text": text_str}]})
                else:
                    text_str = text_str.strip()
                    text_str = '\n' + text_str
                    messages.append({"role": "user", "content": [{"type": "text", "text": text_str}]})
        ...
        if force_final:
            messages.append({"role": "user", "content": [{"type": "text", "text": f"\nPlease provide the final answer now in the format {self.final_token_begin} ... {self.final_token_end}"}]})

        else:
            # If no previous_thoughts but user insists on final
            if force_final:
                messages.append(
                    {
                        "role": "user",
