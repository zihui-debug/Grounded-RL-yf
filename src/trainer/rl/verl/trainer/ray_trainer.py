...
class RayPPOTrainer:
    ...
    def init_workers(self) -> None:
        ...
        if self.config.worker.rollout.multiturn:
            """
            Currently with different amounts of image per sample with multiturn, we need to set
            micro_batch_size_per_device_for_experience to 1.
            TODO: fix this. (this makes training slow)
            """
            # if not self.config.worker.actor.micro_batch_size_per_device_for_experience==1:
            #     raise NotImplementedError("Micro batch size per device for experience must be 1 for now.")
            # if not self.config.worker.actor.micro_batch_size_per_device_for_update==1:
            #     raise NotImplementedError("Micro batch size per device for update must be 1 for now.")
            pass
    ...
    def fit(self):
        ...
        # # Add configuration for reference policy updates
        # ref_update_frequency = getattr(self.config.algorithm, "ref_update_steps", 0)  # 0 means never update
        # last_ref_update_step = 1
        ...
        for _ in tqdm(range(self.config.trainer.total_episodes), desc="Episode", position=0):
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with timer("step", timing_raw):
                    ...
                    batch = batch.repeat(repeat_times=self.n, interleave=True)
                    ...
                    # # Check if it's time to update reference policy
                    # if (ref_update_frequency > 0 and 
                    #     self.use_reference_policy and 
                    #     self.global_step - last_ref_update_step >= ref_update_frequency):
                        
                    #     with timer("update_ref_policy", timing_raw):
                    #         # get latest checkpoint path
                    #         last_global_step_path = os.path.join(self.config.trainer.save_checkpoint_path, CHECKPOINT_TRACKER)
                    #         if os.path.exists(last_global_step_path):
                    #             with open(last_global_step_path, "r") as f:
                    #                 latest_global_step = int(f.read())
                    #             checkpoint_path = os.path.join(self.config.trainer.save_checkpoint_path, f"global_step_{latest_global_step}", "actor")
                    #             print(f"Loading reference policy from checkpoint: {checkpoint_path}")
                    #             self.ref_policy_wg.load_checkpoint(checkpoint_path)
                    #             metrics["ref_policy/updated"] = 1.0
                    #         else:
                    #             print(f"Trying to update ref but no checkpoint found at {last_global_step_path}")
                        
                    #     last_ref_update_step = self.global_step
