...
def postprocess_data(
    ...
    if seq_length < max_length:
        ...
        print(f"Truncating from {seq_length} to {max_length} with truncation method {truncation}.")
...
class AnyPrecisionAdamW(torch.optim.Optimizer):
    ...
    def _zeros_like_param(self, p):      # helper
        return torch.zeros_like(p, dtype=p.dtype)
