...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: RolloutConfig, tokenizer: PreTrainedTokenizer):
        ...
        sampling_kwargs = {
            "max_tokens": config.max_generation_length_per_turn if config.multiturn else config.response_length,
            "detokenize": True if (config.stop_strings and config.multiturn) else False,
            "logit_bias": _get_logit_bias(model_path, trust_remote_code=config.trust_remote_code),
            "stop": config.stop_strings.split(",") if (config.stop_strings and config.multiturn) else None,
            "include_stop_str_in_output": True if (config.stop_strings and config.multiturn) else False,
        }
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        if "multi_modal_data" in non_tensor_batch:
            ...
            for raw_prompt_ids, multi_modal_data in zip(
                ...
                if len(multi_modal_data['image']) > 0:
